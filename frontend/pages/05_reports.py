...
from styles import inject_css, API_BASE, page_header, section_title
...
with right_col:
            ...
            try:
                ...
                if ok:
                    ...
                    if data.get("content"):
                        ...
                        st.info(f"PDF saved server-side: `{data['file_path']}`\nUse the direct download link below.")
                else:
                    # 404 means no analysis has been run yet — surface clearly
                    if "404" in str(resp.status_code) or "not found" in str(data).lower():
    ...
    for fmt_dl, icon, label in [
        ...
        dl_url = f"{API_BASE}/generate-report/download/{session_id}?format={fmt_dl}"
        st.markdown(
