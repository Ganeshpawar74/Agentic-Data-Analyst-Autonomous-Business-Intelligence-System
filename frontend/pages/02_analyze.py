...
for col, prompt in zip(ep_cols, example_prompts):
    with col:
        if st.button(prompt, key=f"ep_{prompt[:20]}", use_container_width=True):
            st.session_state["_prefill_query"] = prompt
            st.rerun()

query_default = st.session_state.pop("_prefill_query", "")
query = st.text_area(
    "Business question",
    value=query_default,
...
with col3:
    run_btn = st.button(
        ...
        disabled=not (query and selected_ids),
    )
...
# ── Run pipeline ───────────────────────────────────────────────────────────────
if run_btn and query and selected_ids:
    status_box = st.empty()
