...
from Agentic_Bi.rag import pipeline, retriever
...
def _set_chroma_client_tmp(monkeypatch, tmp_path):
    ...
    try:
        ...
        def _client_factory():
            return chromadb.PersistentClient(persist_directory=str(tmp_path / "chroma"))

        monkeypatch.setattr(retriever, "CLIENT", _client_factory())
    except Exception:
...
def test_empty_collection_returns_empty_context(tmp_path, monkeypatch):
    ...
    try:
        import chromadb
        client = chromadb.PersistentClient(persist_directory=str(tmp_path / "chroma_empty"))
        # patch module client
